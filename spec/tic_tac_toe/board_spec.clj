  ...
  (context "rotate-cube-x"
    (it "makes a sideways board"
      ...
      (should= [2 11 20 5 14 23 8 17 26 1 10 19 4 13 22 7 16 25 0 9 "X" 3 12 21 6 15 24]
               (sut/rotate-cube-x (populate-board "X" [18] (range 27))))))
    ...
    (context "forward-facing"
      (context "rows"
        (it "matching any row on plane 0 is a win"
          ...
          (should (sut/win? "X" (populate-board "X" [3 4 5] (range 27))))
          (should (sut/win? "X" (populate-board "X" [6 7 8] (range 27))))
          )

        (it "matching any row on plane 1 is a win."
          (should (sut/win? "X" (populate-board "X" [9 10 11] (range 27))))
          (should (sut/win? "X" (populate-board "X" [12 13 14] (range 27))))
          (should (sut/win? "X" (populate-board "X" [15 16 17] (range 27)))))

        (it "matching any row on plane 2 is a win"
          (should (sut/win? "X" (populate-board "X" [18 19 20] (range 27))))
          (should (sut/win? "X" (populate-board "X" [21 22 23] (range 27))))
          (should (sut/win? "X" (populate-board "X" [24 25 26] (range 27)))))
        )

      (context "columns"
        (it "matching any column on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [0 3 6] (range 27))))
          (should (sut/win? "X" (populate-board "X" [1 4 7] (range 27))))
          (should (sut/win? "X" (populate-board "X" [2 5 8] (range 27)))))

        (it "matching any column on plane 1 is a win"
          (should (sut/win? "X" (populate-board "X" [9 12 15] (range 27))))
          (should (sut/win? "X" (populate-board "X" [10 13 16] (range 27))))
          (should (sut/win? "X" (populate-board "X" [11 14 17] (range 27)))))

        (it "matching any column on plane 2 is a win"
          (should (sut/win? "X" (populate-board "X" [18 21 24] (range 27))))
          (should (sut/win? "X" (populate-board "X" [19 22 25] (range 27))))
          (should (sut/win? "X" (populate-board "X" [20 23 26] (range 27)))))
        )

      (context "diagonals"
        (it "matching any diagonal on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [0 4 8] (range 27))))
          (should (sut/win? "X" (populate-board "X" [2 4 6] (range 27)))))

        (it "matching any diagonal on plane 1 is a win"
          (should (sut/win? "X" (populate-board "X" [9 13 17] (range 27))))
          (should (sut/win? "X" (populate-board "X" [15 13 11] (range 27)))))

        (it "matching any diagonal on plane 2 is a win"
          (should (sut/win? "X" (populate-board "X" [18 22 26] (range 27))))
          (should (sut/win? "X" (populate-board "X" [24 22 20] (range 27)))))
    ...
    (context "side-ways facing"
      (context "rows"
        (it "matching row on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [18 9 0] (range 27))))
          (should (sut/win? "X" (populate-board "X" [21 12 3] (range 27))))
          (should (sut/win? "X" (populate-board "X" [24 15 6] (range 27)))))

        (it "matching row on plane 1 is a win"
          (should (sut/win? "X" (populate-board "X" [19 10 1] (range 27))))
          (should (sut/win? "X" (populate-board "X" [22 13 4] (range 27))))
          (should (sut/win? "X" (populate-board "X" [22 13 4] (range 27)))))

        (it "matching row on plane 2 is a win"
          (should (sut/win? "X" (populate-board "X" [20 11 2] (range 27))))
          (should (sut/win? "X" (populate-board "X" [23 14 5] (range 27))))
          (should (sut/win? "X" (populate-board "X" [26 17 8] (range 27)))))
        )

      (context "diagonals"
        (it "matching diagonal on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [18 12 6] (range 27))))
          (should (sut/win? "X" (populate-board "X" [0 12 24] (range 27)))))

        (it "matching diagonal on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [19 13 7] (range 27))))
          (should (sut/win? "X" (populate-board "X" [1 13 25] (range 27)))))

        (it "matching diagonal on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [20 14 8] (range 27))))
          (should (sut/win? "X" (populate-board "X" [2 14 26] (range 27)))))
        )
    ...
    (context "bottom-up facing"
      ...
      (context "diagonals"
        (it "matching diagonals on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [8 16 24] (range 27))))
          (should (sut/win? "X" (populate-board "X" [26 16 6] (range 27)))))

        (it "matching diagonals on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [5 13 21] (range 27))))
          (should (sut/win? "X" (populate-board "X" [23 13 3] (range 27)))))

        (it "matching diagonals on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [2 10 18] (range 27))))
          (should (sut/win? "X" (populate-board "X" [20 10 0] (range 27)))))
        )

      (context "columns"
        (it "matching columns on plane 0 is a win"
          (should (sut/win? "X" (populate-board "X" [8 7 6] (range 27))))
          (should (sut/win? "X" (populate-board "X" [17 16 15] (range 27))))
          (should (sut/win? "X" (populate-board "X" [26 25 24] (range 27)))))

        (it "matching columns on plane 1 is a win"
          (should (sut/win? "X" (populate-board "X" [5 4 3] (range 27))))
          (should (sut/win? "X" (populate-board "X" [14 13 12] (range 27))))
          (should (sut/win? "X" (populate-board "X" [23 22 21] (range 27)))))

        (it "matching columns on plane 2 is a win"
          (should (sut/win? "X" (populate-board "X" [2 1 0] (range 27))))
          (should (sut/win? "X" (populate-board "X" [11 10 9] (range 27))))
          (should (sut/win? "X" (populate-board "X" [20 19 18] (range 27)))))
        )
      )
